...
uses
  ...
  ibex12reader, ibex12writer, ibetypes;
...
type
  TFormX12parser = class(TForm)
    ...
    ibeX12Reader1: TibeX12Reader;
    ibeX12Writer1: TibeX12Writer;
    procedure btnSelectFileClick(Sender: TObject);
    procedure tvwXPathClick(Sender: TObject);
    procedure btnParseClick(Sender: TObject);
    procedure ibeX12Reader1EndFunctionalGroup(Sender: TObject;
      const Tag, ControlNumber: string; Count: Integer;
      const FullSegment: string);
    procedure ibeX12Reader1EndInterchange(Sender: TObject; const Tag,
      ControlNumber, FullSegment: string);
    procedure ibeX12Reader1EndLoop(Sender: TObject);
    procedure ibeX12Reader1EndTransaction(Sender: TObject; const Tag,
      ControlNumber: string; Count: Integer; const FullSegment: string);
    procedure ibeX12Reader1Error(Sender: TObject; ErrorCode: Integer;
      const Description: string);
    procedure ibeX12Reader1ResolveSchema(Sender: TObject;
      const TransactionCode, StandardVersion: string);
    procedure ibeX12Reader1Segment(Sender: TObject; const Tag, Name,
      Loop, FullSegment: string);
    procedure ibeX12Reader1StartFunctionalGroup(Sender: TObject;
      const Tag, ControlNumber, FullSegment: string);
    procedure ibeX12Reader1StartInterchange(Sender: TObject; const Tag,
      ControlNumber, FullSegment: string);
    procedure ibeX12Reader1StartLoop(Sender: TObject;
      const Name: string);
    procedure ibeX12Reader1StartTransaction(Sender: TObject; const Tag,
      ControlNumber, Code, FullSegment: string);
    procedure ibeX12Reader1Warning(Sender: TObject; WarnCode: Integer;
...
begin
   Try
    ...
    begin
     ibeX12Reader1.LoadSchema(txtSchema.Text);
    end;

    if(rbtnFile.Checked) Then
    begin
      ibeX12Reader1.InputFile := txtFile.Text;
    end
    ...
    begin
      ...
      for i := 0 to memoString.Lines.Count -1 do
        ...
        ibeX12Reader1.InputData := inputString;
    end;
...
begin
 ...
 begin
    ...
    numChildren:=  ibeX12Reader1.XChildren;
    ibeX12Reader1.XPath := parentXPath;

    if(ibeX12Reader1.ElementCount > 0) then
    begin
      parent := tvwXPath.Items.AddChild(Node, 'Elements');
      for  i:=0 to ibeX12Reader1.ElementCount-1 do
      begin
         tvwXPath.Items.AddChild(parent, ibeX12Reader1.ElementName[i]+' = '+ibeX12Reader1.ElementValue[i]);
      end;
    end;

    for i:=1 to numChildren  do
    begin
        ibeX12Reader1.XPath := parentXPath+'/['+InttoStr(i)+']';
        buildtree(tvwXPath.Items.AddChild(node,ibeX12Reader1.XSegment));
    end;
 end;
...
procedure TFormX12parser.ibeX12Reader1EndFunctionalGroup(
...
procedure TFormX12parser.ibeX12Reader1EndInterchange(Sender: TObject;
  const Tag, ControlNumber, FullSegment: string);
begin
   memoEvents.Lines.Add('End Interchange: ' + Tag);
end;

procedure TFormX12parser.ibeX12Reader1EndLoop(Sender: TObject);
begin
   memoEvents.Lines.Add('End Loop');
end;

procedure TFormX12parser.ibeX12Reader1EndTransaction(Sender: TObject;
  const Tag, ControlNumber: string; Count: Integer; const FullSegment: string);
begin
   memoEvents.Lines.Add('End Transaction: ' + Tag);
end;

procedure TFormX12parser.ibeX12Reader1Error(Sender: TObject;
  ErrorCode: Integer; const Description: string);
begin
  memoEvents.Lines.Add('ERROR:' + IntToStr(ErrorCode)+':'+Description);
end;

procedure TFormX12parser.ibeX12Reader1ResolveSchema(Sender: TObject;
  const TransactionCode, StandardVersion: string);
begin
  memoEvents.Lines.Add('Resolve Schema: ' + TransactionCode);
end;

procedure TFormX12parser.ibeX12Reader1Segment(Sender: TObject;
  const Tag, Name, Loop, FullSegment: string);
begin
  memoEvents.Lines.Add('Segment: ' + Name);
end;

procedure TFormX12parser.ibeX12Reader1StartFunctionalGroup(
  Sender: TObject; const Tag, ControlNumber, FullSegment: string);
begin
  memoEvents.Lines.Add('Start Functional Group: ' + Tag);
end;

procedure TFormX12parser.ibeX12Reader1StartInterchange(Sender: TObject;
  const Tag, ControlNumber, FullSegment: string);
begin
   memoEvents.Lines.Add('Start Interchange: ' + Tag);
end;

procedure TFormX12parser.ibeX12Reader1StartLoop(Sender: TObject;
  const Name: string);
begin
   memoEvents.Lines.Add('Start Loop: ' + Name);
end;

procedure TFormX12parser.ibeX12Reader1StartTransaction(Sender: TObject;
  const Tag, ControlNumber, Code, FullSegment: string);
begin
  memoEvents.Lines.Add('Start Transaction: ' + Tag);
end;

procedure TFormX12parser.ibeX12Reader1Warning(Sender: TObject;
...
begin
   ...
   begin
      ...
      ibeX12Reader1.XPath := GetXPath(tvwXPath.Selected.Parent);
      txtSegment.Text :=  ibeX12Reader1.XPath;
      txtTag.Text := ibeX12Reader1.XSegment;
      txtValue.Text := ibeX12Reader1.XTag;
      txtElement.Text := '';
   ...
   begin
     ...
     ibeX12Reader1.XPath:=elementXPath;
     txtSegment.Text :=  ibeX12Reader1.XPath;
     txtTag.Text := ibeX12Reader1.XSegment;
     txtValue.Text := ibeX12Reader1.XTag;
     txtElement.Text := '';
