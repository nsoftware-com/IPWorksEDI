...
uses
  ...
  ibex12translator, ComCtrls, ibetypes;
...
type
  TFormX12translator = class(TForm)
    ...
    ibeX12Translator1: TibeX12Translator;
...
begin
  ...
  try
    ibeX12Translator1.Reset();

    if(txtSchema.Text<>'') then
      ibeX12Translator1.LoadSchema(txtSchema.Text);

    if(btnX12File.Checked) then
      ibeX12Translator1.InputFile := txtX12File.Text
    else
      begin
        inputString := '';
      for i := 0 to tMemoX12String.Lines.Count -1 do
        ...
        ibeX12Translator1.InputData:=inputString;
      end;

    if(btnXmlFile.Checked) then
    begin
      ibeX12Translator1.OutputFile := txtXmlFile.Text;
      ibeX12Translator1.Overwrite := chkXmlOverwrite.Checked;
    end;

    ibeX12Translator1.Translate;

    tMemoXmlString.Text := ibeX12Translator1.OutputData;
...
begin
  ...
  try
    ...
    ibeX12Translator1.InputFormat := TibeX12TranslatorInputFormats.xifXML;
    ibeX12Translator1.OutputFormat := TibeX12TranslatorOutputFormats.xofX12;

    if(btnXmlFile.Checked) then
      ibeX12Translator1.InputFile := txtXmlFile.Text
    else
      begin
        inputString := '';
    ...
    if(btnX12File.Checked) then
    begin
      ibeX12Translator1.OutputFile := txtX12File.Text;
      ibeX12Translator1.Overwrite := chkX12Overwrite.Checked;
    end;

    ibeX12Translator1.Translate;

    tMemoX12String.Text := ibeX12Translator1.OutputData;
